...
extern const char notekit_info_plist_section_start __asm("section$start$__TEXT$__info_plist");
...
typedef pid_t (*notekit_resp_pid_fn)(pid_t);

pid_t notekit_responsible_pid(void) {
    static notekit_resp_pid_fn fn = NULL;
    static int resolved = 0;
    if (!resolved) {
        fn = (notekit_resp_pid_fn)dlsym(RTLD_DEFAULT, "responsibility_get_pid_responsible_for_pid");
        resolved = 1;
    }
    if (!fn) return -1;
    return fn(getpid());
...
const void *notekit_embedded_info_plist_bytes(size_t *length) {
    size_t n = (size_t)(&notekit_info_plist_section_end - &notekit_info_plist_section_start);
    if (length) *length = n;
    if (n == 0) return NULL;
    return &notekit_info_plist_section_start;
}
